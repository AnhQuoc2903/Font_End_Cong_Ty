...
}> = ({ open, onClose, loading, result, imageUrl }) => {
  return (
    <Modal
      open={open}
      onCancel={onClose}
        ...
        >
          <div>
            {imageUrl ? (
              ...
              <div
                style={{ width: "100%", height: 240, background: "#f5f5f5" }}
              />
            )}
            <div style={{ marginTop: 12 }}>
              <h4>Labels</h4>
              {result.labels && result.labels.length ? (
                result.labels.map((l) => <Tag key={l}>{l}</Tag>)
              ) : (
                <div style={{ color: "#888" }}>Không tìm thấy</div>
              )}
            </div>
          </div>
          <div>
            <h4>Web Entities</h4>
            {result.webEntities && result.webEntities.length ? (
              result.webEntities.map((w) => (
                <Tag key={w} color="green" style={{ marginBottom: 6 }}>
                  {w}
                </Tag>
              ))
            ) : (
              <div style={{ color: "#888" }}>Không có</div>
            )}
